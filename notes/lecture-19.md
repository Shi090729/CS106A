...
The collection interfaces make the abstraction principle concrete. Code that needs list behavior can use `List` rather than committing itself to `ArrayList`, and code that only needs to iterate can accept an even smaller contract. The implementation can change without rewriting the client algorithm.
...
A map changes the shape of a problem from repeated searching to association. The key must have a stable meaning, and inserting an existing key replaces its old value rather than creating a second entry. A `HashMap` is useful for lookup, but it should not be treated as an ordered sequence.
...
An iterator owns a current position, so calling `next` is both an access and a state change. The lecture also returns to the difference between `==` and `equals`: collections generally need logical equality, especially when strings or separately created objects represent the same value.
